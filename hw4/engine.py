...
class SearchEngine:
    ...
    def read_data(self) -> None:
        ...
        if not self.exists:
            ...
            with open(self.data_path, 'rb') as f:
                self.questions, self.questions_prep, self.answers, bert_vectors, self.bm25_engine = pickle.load(f)
    ...
    def search(self, line: str, mode: str = 'bert', res_line: int = 5, ans_line: int = 5) -> [tuple[list, list], str]:
        """
        ...
        """
        if mode == 'bm25':
            line = self.reader.preprocessor.preprocess_text(line)
        ...
        idx_line = engine.transform([line, ])
